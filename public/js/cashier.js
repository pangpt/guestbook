...
var user = {
	handleTable : function(filter){
		var table = $('#dataTable').DataTable({
			...
			// ajax: '/roles/data',
			ajax: {
                url: baseURL+"/admin/cashier/data?filter="+filter,
                method: 'GET',
            },
			columns: [
				...
				},
				{
					...
					render: function(data, type, row){
						...
						var button = "<button type='button' data-id='"+data.id+"' class='btn dotip btn-success btn-outline btn-circle m-r-5 btn-edt-data' data-toggle='tooltip' title='Edit Admin'>"
									...
									+"</button>"
									// +subbutton
									+"<a data-toggle='modal' data-target='#approveModal'><button type='button' data-url='"+baseURL+"/admin/cashier/approve/"+data.id+"' class='btn dotip btn-info btn-outline btn-circle m-r-5 btn-activate-data' data-toggle='tooltip' title='Approve Admin'>"
										+"<i class='ti-check'></i>"
									+"</button>"
									...
									// +"</button>"
									+"<a data-toggle='modal' data-target='#deleteModal'><button type='button' data-url='"+baseURL+"/admin/cashier/destroy/"+data.id+"' class='btn dotip btn-danger btn-outline btn-circle m-r-5 btn-delete-data' data-toggle='tooltip' title='Delete Admin'>"
										+"<i class='ti-trash'></i>"
									+"</button></a>";
						} else {
						var button = "<button type='button' data-id='"+data.id+"' class='btn dotip btn-success btn-outline btn-circle m-r-5 btn-edt-data' data-toggle='tooltip' title='Edit Admin'>"
									...
									+"</button>"
									// +subbutton
									+"<a data-toggle='modal' data-target='#declineModal'><button type='button' data-url='"+baseURL+"/admin/cashier/decline/"+data.id+"' class='btn dotip btn-warning btn-outline btn-circle m-r-5 btn-decline-data' data-toggle='tooltip' title='Deactivate Admin'>"
										+"<i class='ti-close'></i>"
									+"</button>"
									// +"<button type='button' data-id='"+data.id+"' class='btn btn-info btn-outline btn-circle btn-sm m-r-5 btn-show-permission' data-toggle='tooltip' title='Show Permission'>"
									//	+"<i class='fa fa-certificate'></i>"
		...
		$('#dataForm').validator(['validate']).on('submit', function (e) {
			if (!e.isDefaultPrevented()) {
				...
				if($(this).find("#id").val() == "" && $(this).find("#method").val() === "store"){
					var url = baseURL+"/admin/cashier/store";
				} else if($(this).find("#id").val() != "" && $(this).find("#method").val() === "update"){
					var url = baseURL+"/admin/cashier/update/"+$(this).find("#id").val();
				}
	...
	handleEditData : function(){
		$("#dataTable tbody").on("click", ".btn-edt-data",function(){
			$.ajax({
				url: baseURL+"/admin/cashier/edit/"+$(this).attr("data-id"),
				type: "GET",
				dataType: "JSON",
				success : function(data){
	...
	handleBulkData : function(data, bulkdata){
		...
		$('#btn-bulk').on('click',function(){
			$.ajax({
				url: baseURL+'/admin/cashier/bulk/'+data+'?id='+bulkdata,
				type: 'GET',
				dataType: 'JSON',
				success: function(data){
	...
	handleInfoData : function(){
		$.ajax({
			url: baseURL+"/admin/cashier/info",
			type: 'GET',
			dataType: 'JSON',
			success: function(data){
